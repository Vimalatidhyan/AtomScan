...
This file (RECONX_DOCUMENTATION.md) is deprecated. For all information, please use DOCUMENTATION.md.

## 1. Project Summary
ReconX is a Linux-first Attack Surface Management (ASM) framework that orchestrates multiple recon, OSINT, and vulnerability scanning tools across four phases. A Python orchestrator runs phase-specific Bash modules, parses outputs, and stores structured results in SQLite for querying.
...
Components:
- `reconx.py` orchestrates phases, threads targets, runs Bash modules, and parses outputs.
...
2. `reconx.py` runs the phase module scripts.
...
The runtime behavior is mostly controlled by environment variables and hard-coded defaults in the Bash modules and `reconx.py`.
...
Orchestrator:
- `RECONX_CONTINUE_ON_FAIL`
- `RECONX_PHASE_TIMEOUT`
- `RECONX_PHASE1_TIMEOUT`
- `RECONX_PHASE2_TIMEOUT`
- `RECONX_PHASE3_TIMEOUT`
- `RECONX_PHASE4_TIMEOUT`

Phase 1 tuning:
- `RECONX_WHOIS_TIMEOUT`
- `RECONX_AMASS_INTEL_TIMEOUT`
- `RECONX_AMASS_ENUM_TIMEOUT`
- `RECONX_SUBLIST3R_TIMEOUT`
- `RECONX_SUBFINDER_TIMEOUT`
- `RECONX_SUBFINDER_THREADS`
- `RECONX_ASSETFINDER_TIMEOUT`
- `RECONX_SUBDOMINATOR_TIMEOUT`
- `RECONX_CRTSH_TIMEOUT`
- `RECONX_SECURITYTRAILS_TIMEOUT`
- `RECONX_DNSBRUTER_TIMEOUT`
- `RECONX_DNSPROBER_TIMEOUT`
- `RECONX_DNSX_TIMEOUT`
- `RECONX_DNSX_THREADS`
- `RECONX_HTTPX_TIMEOUT`
- `RECONX_HTTPX_THREADS`
- `RECONX_HTTPX_RUN_TIMEOUT`
- `RECONX_CERTSPOTTER_TIMEOUT`
- `RECONX_CT_MONITOR_TIMEOUT`
- `RECONX_ASNMAP_TIMEOUT`
- `RECONX_MAPCIDR_TIMEOUT`
- `RECONX_CLOUD_ENUM_TIMEOUT`
- `RECONX_S3SCANNER_TIMEOUT`
- `RECONX_GOBLOB_TIMEOUT`
- `RECONX_GCPBRUTE_TIMEOUT`
- `RECONX_CLOUD_THREADS`
- `RECONX_CLOUD_KEYWORDS_LIMIT`

Phase 2 tuning:
- `RECONX_RUSTSCAN_BATCH`
- `RECONX_RUSTSCAN_TIMEOUT`
- `RECONX_SUBJACK_THREADS`
- `RECONX_NMAP_MAX_HOSTS`
- `RECONX_NMAP_HOST_TIMEOUT`
- `RECONX_NMAP_MAX_FILE_MB`
- `RECONX_MIN_DISK_MB`

Phase 3 tuning:
- `RECONX_GAU_THREADS`
- `RECONX_GOSPIDER_THREADS`
- `RECONX_GOSPIDER_CONCURRENCY`
- `RECONX_KATANA_CONCURRENCY`
- `RECONX_FFUF_THREADS`
- `RECONX_FFUF_TIMEOUT`
- `RECONX_FEROX_THREADS`
- `RECONX_DIRSEARCH_THREADS`

Phase 4 tuning:
- `RECONX_THREADS`
- `RECONX_SQLMAP_THREADS`
- `RECONX_VULN_TIMEOUT`
- `RECONX_NUCLEI_RATE_HIGH`
- `RECONX_NUCLEI_RATE_MED`
- `RECONX_NUCLEI_RATE_LOW`
- `RECONX_NUCLEI_RATE_CVE`
- `RECONX_NUCLEI_RATE_MISC`
- `RECONX_SKIPFISH_TIME`
- `RECONX_SKIPFISH_RPS`
- `RECONX_SKIPFISH_MAX_HOSTS`
- `RECONX_SKIPFISH_CONN_GLOBAL`
- `RECONX_SKIPFISH_CONN_PERIP`
- `RECONX_SKIPFISH_REQ_TIMEOUT`
- `RECONX_SKIPFISH_DEPTH`
- `RECONX_SKIPFISH_CHILDREN`
- `RECONX_SKIPFISH_MAX_REQUESTS`
- `RECONX_SKIPFISH_MAX_DESC`
- `RECONX_SKIPFISH_PARTIAL`
- `RECONX_SKIPFISH_RESP_SIZE`
...
## 6. Usage
### 6.1 ReconX Orchestrator
```bash
python3 reconx.py -t example.com
python3 reconx.py -t example.com,example.org -T 5
...
Each phase produces subfolders and raw tool outputs. Parsers read selected files and insert structured records into `reconx.db`.
...
- Phase orchestration and per-target threading in `reconx.py`.
...
- `config.yaml` is never loaded, so all settings in it are ignored. Reference: `/Users/rejenthompson/Documents/technieum-/kali-linux-asm/config.yaml` and `/Users/rejenthompson/Documents/technieum-/kali-linux-asm/reconx.py`.
- The `--resume` CLI flag is parsed but unused. Reference: `/Users/rejenthompson/Documents/technieum-/kali-linux-asm/reconx.py`.
...
ReconX is a security testing framework. Use it only on assets you own or have explicit authorization to test. Unauthorized scanning is illegal in many jurisdictions.

## 14. Troubleshooting
- Missing tools: run `sudo bash install.sh` and ensure `$GOPATH/bin` is in `PATH`.
- SQLite locked: `sqlite3 reconx.db "PRAGMA wal_checkpoint(TRUNCATE);"`

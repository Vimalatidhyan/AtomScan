...
if command -v whois &> /dev/null; then
    ...
    touch "$PHASE_DIR/whois.txt"
    ((TOOLS_SKIPPED++))
fi

# Keep WHOIS artifact present in containerized/minimal environments.
# If whois output is empty, fallback to RDAP over HTTPS.
if [ ! -s "$PHASE_DIR/whois.txt" ]; then
    log_info "WHOIS output empty — attempting RDAP fallback"
    export TARGET
    python3 - << 'WHOISPY' > "$PHASE_DIR/whois.txt" 2>/dev/null || true
import json, os, urllib.request
target = os.environ.get("TARGET", "").strip()
if target:
    req = urllib.request.Request(
        f"https://rdap.org/domain/{target}",
        headers={"User-Agent": "Technieum/1.0"}
    )
    with urllib.request.urlopen(req, timeout=20) as r:
        data = json.loads(r.read().decode("utf-8", errors="ignore"))
    print(f"Domain Name: {target}")
    handle = data.get("handle")
    if handle:
        print(f"Registry Domain ID: {handle}")
    for ent in data.get("entities", []):
        ...
        if isinstance(vcard, list) and len(vcard) == 2 and isinstance(vcard[1], list):
            for row in vcard[1]:
                ...
                val = row[3]
                if key == "fn":
                    print(f"Registrant: {val}")
                elif key == "org":
                    print(f"Registrant Organization: {val}")
                elif key == "email":
                    print(f"Registrar Abuse Contact Email: {val}")
    for ns in data.get("nameservers") or []:
        ldh = ns.get("ldhName")
        if ldh:
            print(f"Name Server: {ldh}")
WHOISPY
fi
...
if ASNMAP_BIN="$(resolve_tool_path asnmap)"; then
    ...
    if [ ! -s "$ASN_CIDRS_FILE" ] || grep -Eiq 'invalid api key|recheck or recreate your api key|enter pdcp api key|could not read input from terminal' "$ASN_DIR/asnmap.err" 2>/dev/null; then
        log_info "asnmap returned no valid CIDRs — using BGPView fallback"
...
if [ -s "$CLOUD_KEYWORDS" ]; then
    ...
    if command -v s3scanner &> /dev/null; then
        ...
        [ "$_s3_ok" -eq 0 ] && log_info "S3Scanner yielded no results or encountered an error"
    else
    ...
    if GOBLOB_BIN="$(resolve_tool_path goblob)"; then
        ...
        [ "$_gb_ok" -eq 0 ] && log_info "goblob yielded no results or unsupported flags on this build"
    else
    ...
    if GCPBRUTE_BIN="$(resolve_tool_path gcpbucketbrute)"; then
        ...
        run_timeout "$GCPBRUTE_TIMEOUT" python3 "$GCPBRUTE_PY" -w "$CLOUD_KEYWORDS" > "$CLOUD_DIR/gcpbucketbrute.txt" 2>/dev/null || log_info "GCPBucketBrute Python mode yielded no results"
    else

...
if command -v nmap &> /dev/null && [ "$TARGETS_COUNT" -gt 0 ]; then
    ...
    if [ -f "$PORTS_DIR/rustscan_ports.txt" ] && [ -s "$PORTS_DIR/rustscan_ports.txt" ]; then
        ...
        # Improve coverage when RustScan is missing: for small target sets, use full scan.
        if [ "$NMAP_TARGET_COUNT" -le 5 ]; then
            NMAP_PORT_FLAG="-p-"
            log_info "RustScan not available; target set is small ($NMAP_TARGET_COUNT), using full Nmap -p-"
        else
...
# Additional OSINT: ASN Lookup (always emit files with layered fallbacks)
touch "$OSINT_DIR/asn_info.txt" "$OSINT_DIR/asn_by_ip.txt" "$OSINT_DIR/asn_summary.txt" "$OSINT_DIR/all_ips.txt"
log_info "Gathering ASN information..."

# Primary: IPs from Phase 1 dnsx JSON
if [ -f "$PHASE1_DIR/dnsx_resolved.json" ]; then
    jq -r '.a[]?' "$PHASE1_DIR/dnsx_resolved.json" 2>/dev/null | sort -u > "$OSINT_DIR/all_ips.txt" || true
fi

# Fallback 1: use resolved port-scan targets
if [ ! -s "$OSINT_DIR/all_ips.txt" ] && [ -f "$PORTS_DIR/targets.txt" ]; then
    grep -E '^([0-9]{1,3}\.){3}[0-9]{1,3}$' "$PORTS_DIR/targets.txt" | sort -u > "$OSINT_DIR/all_ips.txt" || true
fi

# Fallback 2: expand phase1 ASN IP sample if available
if [ ! -s "$OSINT_DIR/all_ips.txt" ] && [ -f "$PHASE1_DIR/asn/asn_ips.txt" ]; then
    head -n 200 "$PHASE1_DIR/asn/asn_ips.txt" | grep -E '^([0-9]{1,3}\.){3}[0-9]{1,3}$' | sort -u > "$OSINT_DIR/all_ips.txt" || true
fi

if command -v whois &> /dev/null && [ -s "$OSINT_DIR/all_ips.txt" ]; then
    # Lookup ASN for each IP (sample first 80 to avoid rate limits)
    # Use timeout 10s per lookup to avoid indefinite stalls.
    head -n 80 "$OSINT_DIR/all_ips.txt" | while IFS= read -r ip; do
        [ -z "$ip" ] && continue
...
elif [ -f "$PHASE1_DIR/asn/asn_cidrs.txt" ] && [ -s "$PHASE1_DIR/asn/asn_cidrs.txt" ]; then
    # Last-resort fallback so API has ASN data even when WHOIS is unavailable.
    head -n 200 "$PHASE1_DIR/asn/asn_cidrs.txt" | awk '{print "cidr:"$1" | unknown | unknown"}' > "$OSINT_DIR/asn_by_ip.txt" 2>/dev/null || true
fi
